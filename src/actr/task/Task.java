...
public class Task extends JPanel {
	...
	/**
	 * Creates an instance of the given task named by its name string. The task name
	 * should be the combined package name and class name for the class that defines
	 * the task -- for example, <tt>"actr.tasks.mypackage.MyTaskClass"</tt>.
	 * 
	...
	/**
	 * Analyzes a batch of tasks and, if desired, outputs the analysis. This method
	 * can be overridden to accept an array of tasks (usually with a length equal to
	 * the number returned by <tt>analysisIterations()</tt>) and perform the desired
	 * analysis over the entire set of tasks. If the <tt>output</tt> parameter is
	 * <tt>true</tt>, the method can use <tt>getModel().output()</tt> to output the
	 * details of the analysis to the screen.
	 * 
	 * @param tasks  the set of tasks to analyze
	 * @param output a flag indicating whether to output the details of analysis to
	 *               the screen
	 ...
	 * update the interface and/or other relevant data structures. The current model
	 * time is passed into the method as an argument.
	 * 
	...
	/**
	 * Finalizes the task. This method can be overridden to allow a custom task to
	 * finalize any necessary data structures and/or perform relevant analysis on
	 * task performance.
	 */
	public void finish() {
	}
	...
	/**
	 * Computes a task-specific evaluation. When the model calls for an evaluation
	 * using the <tt>!eval!</tt> operator, this method is called with an iterator
	 * containing the string tokens in the evaluation (including parentheses as
	 * separate tokens).
	 * 
	...
	/**
	 * Computes a task-specific evaluation condition. When the model calls for an
	 * evaluation using the <tt>!eval!</tt> operator, this method is called with an
	...
	/**
	 * Computes a task-specific binding. When the model calls for an eval binding
	 * using the <tt>!eval!</tt> operator, this method is called with an iterator
	 * containing the string tokens in the evaluation (including parentheses as
	 * separate tokens). The returned value is then bound to the associated variable
	...
	/**
	 * Processes the task display by clearing the model's current visual objects and
	 * then re-adding all existing task components back into the model.
	 */
	...
	/**
	 * Clicks the task mouse at its current location, calling the <tt>doClick()</tt>
	 * method of any buttons at that location.
	 */
	...
	/**
	 * Registers a task keystroke. This method can be overridden to allow a custom
	 * task to act upon model keystrokes.
	 * 
	...
	/**
	 * Adds an aural sound for access by the aural module at the current time. The
	 * identifier should be unique for different sounds; if the same identifier is
	 * used, the new sound will cancel the previously scheduled one.
	 * 
	 * @param id      the unique name of the aural object
	 * @param type    the type of the aural object (i.e., the "kind" slot value)
	 * @param content the content of the aural object (i.e., the "content" slot
	 *                value)
	 */
	...
	/**
	 * Adds an aural sound for access by the aural module at the current time plus
	 * the given time delta. The identifier should be unique for different sounds;
	 * if the same identifier is used, the new sound will cancel the previously
	 * scheduled one.
	 * 
	 * @param timeDelta the future time ahead of the current time at which to
	 *                  schedule the event
	 * @param id        the unique name of the aural object
	 * @param type      the type of the aural object (i.e., the "kind" slot value)
	 * @param content   the content of the aural object (i.e., the "content" slot
	 ...
	 * spotlight, and eye location. This method should not normally be overridden.
	 * 
	 * @param g the graphics display
	 */
